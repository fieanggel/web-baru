...
async function createDeposit({ userId, categoryId, estimatedWeight }, conn = pool) {
  const id = randomUUID()
  await conn.query(
    'INSERT INTO deposits (id, user_id, category_id, estimated_weight, status) VALUES (?, ?, ?, ?, ?)',
    [id, userId, categoryId, estimatedWeight, 'PENDING'],
  )
